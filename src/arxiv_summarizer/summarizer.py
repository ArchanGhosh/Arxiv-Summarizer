...
from arxiv_summarizer.utils import chunk, is_arxiv_identifier, strip


class SummarizationModel:
    """Summarization model for generating summaries."""

    def __init__(self, model, tokenizer, as_cli=False, **kwargs) -> None:
        ...
        self.model = [
            lambda x_and_kwargs: piped_model(x_and_kwargs[0], **x_and_kwargs[1])
        ]
    ...
    def __call__(self, text: str) -> List[Any]:
        ...
        Returns:
            List[str]: The generated summary.
        """
            ...
            for layer in self.model:
                __data__ = layer(
                    (
                        ...
                        {
                            "max_length": min(
                                self.kwargs["max_length"], len(data)
                            ),  # At most, the size of the input text
                            "min_length": min(
                                self.kwargs["min_length"], (len(data) * 2) // 3
                            ),  # At most 66.6% of the size of the input text
...
class ArxivSummarizer:
    ...
    def __init__(
        self,
        summarizer: SummarizationModel = None,
        model: str = None,
        max_length: int = 256,
        min_length: int = 64,
    ...
    def __call__(
        self, arxiv_id: str
    ) -> Tuple[ArxivPaper, Union[AnyStr, List[str], bytes]]:
        ...
        if len(papers) == 0:
            if is_arxiv_identifier(arxiv_id):
                ...
                raise ArxivQueryReturnedEmpty(
                    f"Cannot find any arxiv entry containing `{arxiv_id}`"
                )
